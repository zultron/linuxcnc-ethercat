...
int lcec_el5122_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  ...
  for (i=0; i<LCEC_EL5122_CHANS; i++) {
    ...
    LCEC_PDO_INIT(pdo_entry_regs, slave->alias, slave->index, slave->vid, slave->pid, 0x6000 + (i << 4), 0x04, &chan->undeflow_pdo_os, &chan->overflow_pdo_bp);
    LCEC_PDO_INIT(pdo_entry_regs, slave->alias, slave->index, slave->vid, slave->pid, 0x6000 + (i << 4), 0x05, &chan->overflow_pdo_os, &chan->overflow_pdo_bp);
    ...
    LCEC_PDO_INIT(pdo_entry_regs, slave->alias, slave->index, slave->vid, slave->pid, 0x6000 + (i << 4), 0x0c, &chan->gate_state_pdo_os, &chan->gate_state_pdo_bp);
